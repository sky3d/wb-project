...
export const Tooltype: FC<TTooltype> = ({ children, msgChildren, stylePopup = {} }): JSX.Element => {
  ...
  useEffect(() => {
    ...
    setPopupStyle({ marginTop: -msg.current.offsetHeight, '--wp': `${msg.current.offsetWidth / 2}px` })

    // const rootMouseMove = (e) => {
    //   setPopupStyle({
    //     marginTop: -msg.current.offsetHeight, '--wp': `${msg.current.offsetWidth / 2}px`, top: root.current.offsetTop, visibility: 'visible',
    //     opacity: 1
    //   })
    // }
